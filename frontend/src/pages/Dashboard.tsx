...
const Dashboard = () => {
  ...
  return (
    <div className={`min-h-screen p-8 ${isDark
        ? 'bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950'
        : 'bg-gradient-to-br from-gray-100 via-white to-gray-100'
      }`}>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className={`text-3xl font-bold ${isModern ? 'modern-glow-text' : (isDark ? 'text-white' : 'text-gray-900')
            }`}>
            Dashboard
          </h1>
          <button
            onClick={handleLogout}
            className={`px-6 py-2 rounded-lg transition-all ${isModern
        ...
        <div className={`rounded-2xl p-8 ${isModern
            ? 'liquid-glass'
            : `glass-effect ${isDark ? '' : 'bg-white/80 border-gray-200/50'}`
          }`}>
          <h2 className={`text-2xl font-semibold mb-4 ${isModern && isDark ? 'modern-glow-text' : (isDark ? 'text-white' : 'text-gray-900')
            }`}>
            Welcome to Nayte!
          </h2>
          ...
          {user && (
            <div className={`mt-4 p-4 rounded-xl ${isModern
                ? 'modern-profile-card'
